...
npm run provider -- discord            # regenerate this repo
npm run provider -- discord --check    # exit 1 on any difference
```
...
| `packages/ui` | `@particle-academy/discord-ui` — the authoring surface, React, every host | Generated |
| `packages/js` | `@particle-academy/discord-js` — Node, on `@particle-academy/fancy-connector-core` | Generated |
| `packages/php` | `particle-academy/discord-php` — PHP 8.4, on `particle-academy/fancy-connector-core` | Generated |
| `packages/py` | `fancy-discord` — Python 3.11+, stdlib only | Generated |

The authoritative copy of `provider/` lives in the weaver envelope at
`providers/discord/provider/`. The copy here is emitted from it and `--check`
...
| | Namespace |
|---|---|
| This repo | `github.com/Fancy-Friends/discord` |
| npm | `@particle-academy/discord-ui`, `@particle-academy/discord-js` |
| Packagist | `particle-academy/discord-php` |
| PyPI | `fancy-discord` |
...
**Default to plain HTTP.** A Discord SDK is third-party code and is subject to
