/*
 * Discord — the published npm packages.
 *
...
import { discordFaker } from "@particle-academy/discord-js";
import { DISCORD_KINDS } from "@particle-academy/discord-ui";
 ...
 */
const resolved = import.meta.resolve("@particle-academy/discord-js");
...
for (const { operation, config, expected } of GOLDENS) {
  const faked = discordFaker(operation, fakeRequest("discord", operation, config));
...
assert.equal(DISCORD_KINDS.length, 1);
for (const kind of DISCORD_KINDS) {
...
console.log(`  ok   ui kinds resolve from ${"@particle-academy/discord-ui"}`);
